...
## RLS: reordenar un UPDATE antes de una lectura puede bloquear esa misma lectura sin avisar

Al hacer atómico `acceptRoutineShare` (evitar que dos llamadas concurrentes creen dos copias de una rutina compartida, ver `docs/agents/accept-routine-share-atomic-status.md`), la solución obvia era mover el `UPDATE` condicional que "reclama" la propuesta (`status: 'pending' → 'accepted'`) al principio de la función, antes de leer y copiar la rutina — así solo quien gana ese `UPDATE` sigue adelante. El código compiló limpio, pasó dos rondas de revisión de código (spec compliance + calidad), y **rompía la función por completo en cada llamada real**: la política de RLS que le da al receptor permiso de `select` sobre la rutina compartida (`"El receptor de una rutina compartida pendiente puede verla"`) exige `routine_shares.status = 'pending'` — con el `UPDATE` corrido primero, para cuando la función intentaba leer la rutina esa condición ya era falsa, RLS bloqueaba el `select` en silencio (`maybeSingle()` devuelve `null`, no un error), y la función tiraba "No se encontró la rutina compartida." siempre, revirtiendo el reclamo cada vez.

Ninguna revisión de código (ni la propia, ni dos rondas de subagentes) detectó esto — hace falta cruzar el código en `src/lib/*.ts` contra la política de RLS específica en `supabase/schema.sql`, algo que ninguna lectura aislada del `.ts` revela. Recién apareció al correr el flujo completo contra Supabase real con Playwright: la propuesta nunca pasaba a "aceptada" de verdad, pese a que el build y el `tsc` estaban limpios y las dos revisiones habían aprobado el código.

**La lección general:** cuando una fila tiene una política de RLS de `select` condicionada a una columna que una función también escribe (ej. `status`), el orden importa de una forma que no es visible leyendo solo el código TypeScript — hay que decidir explícitamente si cada lectura necesita pasar *antes* o *después* de cada escritura, no solo qué orden cierra la carrera de concurrencia que se está resolviendo. En este caso la solución fue separar los dos problemas: las lecturas (sin efectos secundarios, no reabren ninguna carrera) se quedan en su posición original, *antes* de cualquier escritura; solo el `UPDATE` que sí necesita ser atómico se reordena, y se reordena relativo al `INSERT` que crea el dato, no relativo a las lecturas. Cualquier reordenamiento de escrituras en una función con políticas de RLS condicionadas por estado necesita, como mínimo, una corrida real contra Supabase (no alcanza con revisión de código) antes de darse por cerrado.
