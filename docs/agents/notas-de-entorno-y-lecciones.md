...
El patrón documentado arriba (`supabase db query --linked "UPDATE auth.users SET encrypted_password = crypt(...) ..."`) funciona en sesiones interactivas normales, pero **en una sesión con auto-mode activo el clasificador de permisos lo puede bloquear igual**, tratándolo como una acción sensible sobre credenciales aunque el comando en sí no contenga ningún secreto en texto plano (no es el mismo caso que el bloqueo de `curl` con token crudo de la sección de arriba — acá se bloquea el propio `UPDATE` sobre `auth.users`, corriendo por el CLI de `supabase`, que en otros contextos sí está permitido).

No tiene sentido buscarle la vuelta al bloqueo (probar variantes del comando, otro camino para lograr lo mismo) — el mensaje del clasificador ya lo dice explícitamente: si la acción es esencial, parar y explicarle a la usuaria qué se necesita y por qué, para que decida. En la práctica la salida más simple es pedirle a la usuaria que corra ella misma el mismo comando anteponiendo `!` en su prompt — eso lo ejecuta directamente en su sesión, sin pasar por el clasificador de auto-mode, y el resultado (contraseña reseteada) es idéntico. Después de eso, el resto del flujo de Playwright contra la cuenta de prueba sigue exactamente igual que siempre.
